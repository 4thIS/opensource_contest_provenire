...
| 2 | **`scan --diff` 가 한글 주석만 있어도 크래시** | `subprocess(text=True)` 에 encoding 미지정 | **이슈 #22 — 아직 미해결** |
| 3 | `twine` 이 `~/.pypirc` 를 못 읽음 | 설정 파일에 한글 주석 | `.pypirc` 를 ASCII로 재작성 |
| 4 | `twine` 출력이 죽음 | rich 라이브러리가 `•` 출력 | `PYTHONIOENCODING=utf-8` |

**2번이 특히 아프다.** 우리 타겟 사용자는 한국 개발자고, 한글 주석은 예외가 아니라 기본값이다.
...
### ③ 사람이 실행하는 스크립트는 stdout을 고정

`benchmarks/*.py` 처럼 **직접 실행되고 한글을 출력하는** 스크립트는 맨 위에:
...
| **`src/provenire/adapters/git.py`** | ❌ **이슈 #22 — 미해결** |
| `tests/test_git_adapter.py` 의 `_git()` 헬퍼 | ⚠️ 같은 패턴 (테스트용이라 영향 작음) |
