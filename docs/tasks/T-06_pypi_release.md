...
- [ ] **TestPyPI 에 먼저 올려** 설치까지 확인한다 (아래 주의 참조)
- [ ] PyPI 에 **0.1.0 정식 배포**
...
core/ index/ benchmarks/ 는 읽기만 해. PyPI 실제 업로드는 내가 직접 할 테니 그 전까지 준비해줘.
```

> ⚠️ **AI에게 PyPI 업로드(`twine upload`)를 맡기지 마라.** 토큰이 필요하고 되돌릴 수 없다.
> 빌드·검증까지만 AI 와 하고, **업로드는 사람이 직접** 한다.
