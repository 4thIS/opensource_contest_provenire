...
> 🔴 **빌드 전에 `dist/` 를 반드시 비워라.**
> `twine upload dist/*` 는 **디렉터리 안의 모든 파일을 올린다.** 이전 빌드(0.0.1 등)가 남아 있으면
> 같이 업로드를 시도해 **에러로 배포가 중단**된다. 팀장이 토큰 검증하면서 실제로 겪었다:
> ```
> 400 File already exists ('provenire-0.0.1-py3-none-any.whl')
> ```
> 검증 목적이라 문제없었지만, 정식 배포 때 이러면 흐름이 끊긴다. **빌드마다 `rm -rf dist/` 부터.**

### 💡 Windows 인코딩 주의 (겪게 될 확률 높음)

우리 프로젝트에서 **세 번 반복된 문제**다 — 한국어 Windows 콘솔이 `cp949` 라서 생긴다.

```bash
# twine 출력이 UnicodeEncodeError 로 죽으면
PYTHONIOENCODING=utf-8 python -m twine upload dist/*
```

또 하나: **`~/.pypirc` 에 한글(주석 포함)을 넣지 마라.** twine 이 그 파일을 시스템 인코딩으로
읽어서 `UnicodeDecodeError` 로 죽는다. **ASCII 로만** 작성한다.
...
> 팀장이 머지 후 아래로 정식 배포하고, `pip install --upgrade provenire` 로 확인한다.
> ```bash
> rm -rf dist/                                          # 이전 빌드 제거 (필수)
> python -m build
> PYTHONIOENCODING=utf-8 python -m twine upload dist/*  # Windows 인코딩 대비
> ```
>
> 참고: 팀장 토큰은 **검증 완료**됐다(2026-07-20). 이미 존재하는 0.0.1 업로드를 시도해
> `400 File already exists` 응답을 받았다 — `403` 이 아니므로 **인증·업로드 권한 정상**이다.
